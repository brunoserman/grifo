...
//
// q is optional: picking a scope or a tag with no text is "browse this
// filter" rather than "search nothing", and lists the most recent matches
// (item.excerpt, or the highlight's own text, stands in for the snippet
// since there's no match to center it on).
search.get('/search', async (c) => {
  const match = buildMatchQuery(c.req.query('q') ?? '')
  const scope = c.req.query('scope') ?? 'all'
  const tag = c.req.query('tag')?.trim() || null
  if (!match && scope === 'all' && !tag) return c.json({ results: [] })
  ...
  if (match) {
    ...
    if (includeItems) {
      parts.push(`
        SELECT
          'item' AS kind, i.id AS item_id, i.title AS title, i.type AS type,
          i.author AS author, i.site_name AS site_name, NULL AS highlight_id,
          snippet(items_fts, -1, char(57344), char(57345), '…', 12) AS snippet,
          bm25(items_fts) AS sort_key
        FROM items_fts
    ...
    if (includeHighlights) {
      parts.push(`
        SELECT
          'highlight' AS kind, h.item_id AS item_id, i.title AS title, i.type AS type,
          i.author AS author, i.site_name AS site_name, h.id AS highlight_id,
          snippet(highlights_fts, -1, char(57344), char(57345), '…', 12) AS snippet,
          bm25(highlights_fts) AS sort_key
        FROM highlights_fts
    ...
    // Browse mode: no MATCH clause, most recent first (sort_key is negated so
    // the shared "ORDER BY sort_key ASC" below still puts the newest on top).
    if (includeItems) {
      parts.push(`
        SELECT
          'item' AS kind, i.id AS item_id, i.title AS title, i.type AS type,
          i.author AS author, i.site_name AS site_name, NULL AS highlight_id,
          COALESCE(i.excerpt, substr(i.content_text, 1, 160), '') AS snippet,
          -i.saved_at AS sort_key
        FROM items i
        WHERE 1=1${itemStatus ? ' AND i.status = ?' : ''}${tagClause('i.id')}
      `)
      if (itemStatus) binds.push(itemStatus)
      if (tag) binds.push(tag)
    }
    if (includeHighlights) {
      parts.push(`
        SELECT
          ...
          i.author AS author, i.site_name AS site_name, h.id AS highlight_id,
          h.text AS snippet,
          -h.created_at AS sort_key
        FROM highlights h
        JOIN items i ON i.id = h.item_id
        WHERE 1=1${tagClause('h.item_id')}
      `)
      if (tag) binds.push(tag)
    }
  }
